function [out, valid] = serializeBusWithDataLoggerHeader_ContinuousDataBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('ContinuousDataBus')
    ...
    coder.varsize('out', 61739 + headerLength);
    outSize = headerLength + getSerializedBusLength_ContinuousDataBus(bus, namePrefix);
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(753309036), uint16(2), timestamp);
