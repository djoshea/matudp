...
%   writeDeserializeBusModelPackagedCode('testBusArray', 'TestBus')
    ...
    if valid == uint8(0) || ~isequal(in(offset:offset+uint16(21-1)), ...
        uint8([1, 4, 0, typecast(uint16(10), 'uint8'), 'centerSize', typecast(uint16(2), 'uint8'), 'mm', 5, 2]'))
        valid = uint8(0);
    end
    offset = offset + uint16(21);

    % Establishing size
    coder.varsize('bus.centerSize', [3 3]);
    if valid == uint8(0) || offset + uint16(4 - 1) > numel(in)
        % buffer not large enough
        valid = uint8(0);
        bus.centerSize = zeros([3 3], 'uint16');
    else
        sz = typecast(in(offset:(offset+uint16(4-1))), 'uint16')';
        offset = offset + uint16(4);
        % check size
        if sz(1) > uint16(3), valid = uint8(0); end
        if sz(2) > uint16(3), valid = uint8(0); end
        elements = uint16(1);
        for i = 1:2
        ...
        if valid == uint8(0) || offset + uint16(elements*2 - 1) > numel(in)
            % buffer not large enough
            valid = uint8(0);
            bus.centerSize = zeros([1 1], 'uint16');
        else
            % mollify codegen
            assert(sz(1) <= uint16(3));
            assert(sz(2) <= uint16(3));
            % read and typecast data
            assert(elements <= uint16(9));
            bus.centerSize = zeros(sz, 'uint16');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Deserializing fixed-sized field enum
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if valid == uint8(0) || ~isequal(in(offset:offset+uint16(17-1)), ...
        uint8([0, 4, 0, typecast(uint16(4), 'uint8'), 'enum', typecast(uint16(4), 'uint8'), 'enum', 8, 1]'))
        valid = uint8(0);
    end
    ...
    if valid == uint8(0) || offset + uint16(2 - 1) > numel(in)
        ...
        % check size
        if sz(1) > uint16(9), valid = uint8(0); end % max enum member length as string
        elements = uint16(1);
        for i = 1:1
             elements = elements * uint16(sz(i));
        end
        if valid == uint8(0) || offset + uint16(elements*1 - 1) > numel(in)
            ...
            bus.enum = TestEnum.val1;
            if elements > uint16(0)
                [bus.enum, valueValid] = testBusArrayLib.stringToEnum_TestEnum(typecast(in(offset:offset+uint16(elements*1 - 1)), 'uint8'));
