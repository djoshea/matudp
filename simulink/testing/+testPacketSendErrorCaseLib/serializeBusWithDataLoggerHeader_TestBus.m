...
%   writeSerializeBusModelPackagedCode('testPacketSendErrorCase', 'TestBus')

    valid = uint8(0);
    headerLength = uint16(BusSerialize.computeDataLoggerHeaderLength(groupName));
    coder.varsize('out', 124 + headerLength);
    outSize = headerLength + testPacketSendErrorCaseLib.getSerializedBusLength_TestBus(bus);
    out = zeros(outSize, 1, 'uint8');
    offset = uint16(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, groupName, uint32(3693062004), uint16(3), timestamp);
