...
%   writeSerializeBusModelPackagedCode('testPacketSendErrorCase', 'TestBusOuter')

    valid = uint8(0);
    headerLength = uint16(BusSerialize.computeDataLoggerHeaderLength(groupName));
    outSize = headerLength + testPacketSendErrorCaseLib.getSerializedBusLength_TestBusOuter(bus);
    out = zeros(outSize, 1, 'uint8');
    offset = uint16(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, groupName, uint32(812220335), uint16(5), timestamp);
    ...
    nestedBytes = uint16(testPacketSendErrorCaseLib.getSerializedBusLength_TestBusInner(bus.nested(1)));
    out(offset:(offset+nestedBytes-uint16(1))) = testPacketSendErrorCaseLib.serializeBus_TestBusInner(bus.nested(1));
