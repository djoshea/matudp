...
%   BusSerialize.writeSerializeBusModelPackagedCode('testDataLoggerLocal', 'TestBusOuter')

    valid = uint8(0);
    headerLength = uint16(BusSerialize.computeDataLoggerHeaderLength(groupName));
    outSize = headerLength + testDataLoggerLocalLib.getSerializedBusLength_TestBusOuter(bus);
    ...
    nestedBytes = uint16(testDataLoggerLocalLib.getSerializedBusLength_TestBusInner(bus.nested(1)));
    out(offset:(offset+nestedBytes-uint16(1))) = testDataLoggerLocalLib.serializeBus_TestBusInner(bus.nested(1));
