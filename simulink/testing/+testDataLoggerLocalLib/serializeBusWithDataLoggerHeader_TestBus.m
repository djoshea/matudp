...
%   BusSerialize.writeSerializeBusModelPackagedCode('testDataLoggerLocal', 'TestBus')

    valid = uint8(0);
    headerLength = uint16(BusSerialize.computeDataLoggerHeaderLength(groupName));
    coder.varsize('out', 6491 + headerLength);
    outSize = headerLength + testDataLoggerLocalLib.getSerializedBusLength_TestBus(bus);
    out = zeros(outSize, 1, 'uint8');
    offset = uint16(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, groupName, uint32(2205984824), uint16(3), timestamp);
