function [out, valid] = serializeBusWithDataLoggerHeader_TestBus(bus, groupType, groupName, groupMeta)

    valid = uint8(0);
    headerLength = uint16(BusSerialize.computeDataLoggerHeaderLength(groupName, groupMeta));
    coder.varsize('out', 124 + headerLength);
    outSize = headerLength + testPacketSendLocalLib.getSerializedBusLength_TestBus(bus);
    out = zeros(outSize, 1, 'uint8');
    offset = uint16(1);

        header = BusSerialize.serializeDataLoggerHeader(groupType, groupName, groupMeta, uint32(3693062004), uint16(3));
