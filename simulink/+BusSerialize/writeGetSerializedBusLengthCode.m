...
% reflecting the current sizes of variable-sized contents
    ...
    w('%%   writeGetSerializedBusLengthCode(''%s'')\n', busName);
    w('\n');
    ...
    for i = 1:numel(elements)
        ...
        if isBus
            ...
            for iElement = 1:numElements
                w('    outSize = outSize + uint32(%s(bus.%s(%d), uint8([namePrefix ''_'' %s]))); %% for %s nested bus\n', lenFnName, e.Name, iElement, e.Name, e.Name);
            end
        else
            % not a nested bus
            ...
            if signalSpec.isEnum
                ...
                w('    outSize = outSize + uint32(numel(%s(bus.%s))); %% for %s enum to string\n', enumFnName, e.Name, e.Name);
