...
%             w('\n');
            ...
            if signalSpec.isEnum
                ...
                w('    %s = uint8(%s(bus.%s))''; \n', enumStrVarName, enumFnName, e.Name); % convert to column vector
            ...
            for iSubElement = 1:numElements
                ...
                if includeDataLoggerHeader
                    ...
                    if numElements > 1
                        ...
                        w('    subNamePrefix = uint8([namePrefixBytes, uint8(''_''), uint8(''%d''), uint8(''_''), uint8(''%s'')]);\n', iSubElement, e.Name);
                    else
                        w('    subNamePrefix = uint8([namePrefixBytes, uint8(''_''), uint8(''%s'')]);\n', e.Name);
                    end
