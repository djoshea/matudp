function writeEnumVectorToSemicolonDelimitedStringCode(enumName)
...
% package.
    ...
    fnName = sprintf('enumVectorToSemicolonDelimitedString_%s', enumName);
    ...
    w('function str = %s(enumVector)\n', fnName);
    w('%% DO NOT EDIT: Auto-generated by \n');
    w('%%   BusSerialize.writeEnumToStringCode(''%s'')\n', enumName);
    w('%%   Not compatible with code gen!\n');
    w('\n');
    
    w('    valid = uint8(1);\n');
     
   % w('    coder.varsize(''str'', [1 %d], [false true]);\n', maxLength * numel(enumVector));
   
    w('    str = strjoin(arrayfun(@char, enumVector, ''UniformOutput'', false), '';'');\n');
%    
%    
