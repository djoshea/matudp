...
%
    ...
    for iElement = 1:numel(elements)
        ...
        if ~isBus
            ...
            % concatenation dimension, name, units,
            % data type id, and nDims
            nBytes = 1 + 1 + 1 + 2 + numel(e.Name) + 2 + numel(e.DocUnits) + 1 + 1;
            w('    if valid == uint8(0) || offset + uint16(%d - 1) > numel(in)\n', nBytes);
            w('        valid = uint8(0);\n');
            w('    end\n');
            ...
            w('    if valid == uint8(0) || ~isequal(in(offset:offset+uint16(%d-1)), ...\n', nBytes);
            w('        uint8([%d, %d, %d, typecast(uint16(%d), ''uint8''), ''%s'', typecast(uint16(%d), ''uint8''), ''%s'', %d, %d]''))\n', ...
                bitFlags, sigType, concatDim, numel(e.Name), e.Name, ...
                numel(e.DocUnits), e.DocUnits, dtid, ndims);
            w('        valid = uint8(0);\n');
            w('    end\n');
            w('    offset = offset + uint16(%d);\n', nBytes);
            w('\n');
            ...
            w('        %% buffer not large enough\n');
            w('        valid = uint8(0);\n');
           %w('        bus.%s = zeros(%s, ''%s'');\n', e.Name, mat2str(ones(1, min(ndims, 2))), e.DataType);
            if signalSpec.isEnum
            ...
            w('        sz = typecast(in(offset:(offset+uint16(%d-1))), ''uint16'')'';\n', bytesSize);
            w('        offset = offset + uint16(%d);\n', bytesSize);
            ...
            if signalSpec.isEnum
                w('            bus.%s = %s.%s;\n', e.Name, signalSpec.enumName, signalSpec.getEnumDefaultValueAsString());
            elseif isVariable
                w('            bus.%s = zeros(%s, ''%s'');\n', e.Name, mat2str(ones(1, max(2, ndims))), e.DataType);
            else
                w('            bus.%s = zeros(%s, ''%s'');\n', e.Name, dimsStr, e.DataType);
            end
            w('        else\n');
            ...
            if signalSpec.isEnum
                ...
                w('                [bus.%s, valueValid] = %s(typecast(in(offset:offset+uint16(elements*%d - 1))'', ''uint8'')'');\n', ...
                ...
                if forMatlab
                    ...
                    w('                    bus.%s = typecast(in(offset:offset+uint16(elements*%d - 1)), ''char'');\n', e.Name, bytesPerElement);
                    w('                end\n');
                else
                ...
                if forMatlab
                    ...
                    if signalSpec.isLogical
                        w('                bus.%s = logical(in(offset:offset+uint16(elements*%d - 1)));\n', ...
                            e.Name, bytesPerElement);
                    elseif signalSpec.isChar
                        w('                bus.%s = char(in(offset:offset+uint16(elements*%d - 1)));\n', ...
                            e.Name, bytesPerElement);
                        % transpose into row vector
                        w('                if size(bus.%s, 1) > size(bus.%s, 2) && size(bus.%s, 2) == 1, bus.%s = bus.%s''; end\n', ...
                            e.Name, e.Name, e.Name, e.Name, e.Name);
                    else
                        w('                bus.%s(1:elements) = typecast(in(offset:offset+uint16(elements*%d - 1))'', ''%s'')'';\n', ...
            ...
            w('                offset = offset + uint16(elements*%d);\n', bytesPerElement);
            w('            end\n');
            w('        end\n');
            w('    end\n');
            ...
            if numElements > 1
                ...
                w('    iSubElement = uint16(2);\n');
                w('    while iSubElement <= uint16(%d) && valid == uint8(1)\n', numElements);
                % this element is okay, actually deserialize it
                w('        [bus.%s(iSubElement), valid, offset] = %s(in, offset, valid);\n', ...
                    e.Name, deserializeFnName);
                w('        iSubElement = iSubElement + uint16(1);\n');
                w('    end\n');
